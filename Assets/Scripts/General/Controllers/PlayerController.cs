...
using Shadowfinder.Scene;
using Shadowfinder.Gathering;
using Shadowfinder.UI;
using Shadowfinder.Weapons;

namespace Shadowfinder.Controllers
{
    public class PlayerController : TickNetworkBehaviour
    {
        ...
        float _invenTimer = 0f;
        int _treeType = 1;

        Vector3 _lastdir = Vector3.zero;
        Vector3 _lastpos = Vector3.zero;

        CanvasGroupFader _invenFader;
        ...
        void Update()
        {
            ...
            {
                if (Time.fixedTime - _invenTimer > 0.25f)
                {
                    _invenTimer = Time.fixedTime;
                    if (_invenFader.IsVisible) _invenFader.Hide();
                    else _invenFader.Show();
                }
            }
        ...
        {
            var cinven = GameObject.Find("Inventory Window");
            if (cinven != null) _invenFader = cinven.GetComponent<CanvasGroupFader>();
